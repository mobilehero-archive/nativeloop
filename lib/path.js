    ...
    for (var i = 0; i <= path.length; ++i) {
        ...
        if (47 === code || 92 === code) {
            if (lastSlash === i - 1 || 1 === dots) ; else if (lastSlash !== i - 1 && 2 === dots) {
                if (res.length < 2 || 46 !== res.charCodeAt(res.length - 1) || 46 !== res.charCodeAt(res.length - 2)) if (res.length > 2) {
                    var start = res.length - 1;
                    var j = start;
                    for (;j >= 0; --j) if (92 === res.charCodeAt(j)) break;
                    if (j !== start) {
        ...
        if (47 === code) {
            if (lastSlash === i - 1 || 1 === dots) ; else if (lastSlash !== i - 1 && 2 === dots) {
                if (res.length < 2 || 46 !== res.charCodeAt(res.length - 1) || 46 !== res.charCodeAt(res.length - 2)) if (res.length > 2) {
                    var start = res.length - 1;
                    var j = start;
                    for (;j >= 0; --j) if (47 === res.charCodeAt(j)) break;
                    if (j !== start) {
                        res = -1 === j ? "" : res.slice(0, j);
                        lastSlash = i;
                        dots = 0;
...
var win32 = {
    resolve: function() {
        ...
        for (var i = arguments.length - 1; i >= -1; i--) {
            var path;
            if (i >= 0) path = arguments[i]; else if (resolvedDevice) {
    ...
    relative: function(from, to) {
        ...
        if (from === to) return "";
        var fromStart = 0;
        for (;fromStart < from.length; ++fromStart) if (92 !== from.charCodeAt(fromStart)) break;
        var fromEnd = from.length;
        for (;fromEnd - 1 > fromStart; --fromEnd) if (92 !== from.charCodeAt(fromEnd - 1)) break;
        var fromLen = fromEnd - fromStart;
        var toStart = 0;
        for (;toStart < to.length; ++toStart) if (92 !== to.charCodeAt(toStart)) break;
        var toEnd = to.length;
        for (;toEnd - 1 > toStart; --toEnd) if (92 !== to.charCodeAt(toEnd - 1)) break;
        ...
        if (void 0 !== ext && ext.length > 0 && ext.length <= path.length) {
            ...
            for (i = path.length - 1; i >= start; --i) {
                ...
                if (47 === code || 92 === code) {
                    ...
                    if (extIdx >= 0) if (code === ext.charCodeAt(extIdx)) -1 === --extIdx && (end = i); else {
                        extIdx = -1;
    ...
    parse: function(path) {
        ...
        if (-1 === startDot || -1 === end || 0 === preDotState || 1 === preDotState && startDot === end - 1 && startDot === startPart + 1) -1 !== end && (ret.base = ret.name = 0 === startPart && isAbsolute ? path.slice(rootEnd, end) : path.slice(startPart, end)); else {
...
var posix = {
    ...
    relative: function(from, to) {
        ...
        if (from === to) return "";
        var fromStart = 1;
        for (;fromStart < from.length; ++fromStart) if (47 !== from.charCodeAt(fromStart)) break;
        var fromEnd = from.length;
        var fromLen = fromEnd - fromStart;
        var toStart = 1;
        for (;toStart < to.length; ++toStart) if (47 !== to.charCodeAt(toStart)) break;
        var toEnd = to.length;
        var toLen = toEnd - toStart;
        var length = toLen > fromLen ? fromLen : toLen;
        var lastCommonSep = -1;
    ...
    parse: function(path) {
        ...
        if (-1 === startDot || -1 === end || 0 === preDotState || 1 === preDotState && startDot === end - 1 && startDot === startPart + 1) -1 !== end && (ret.base = ret.name = 0 === startPart && isAbsolute ? path.slice(1, end) : path.slice(startPart, end)); else {
            if (0 === startPart && isAbsolute) {
